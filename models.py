...
class User(UserMixin, db.Model):
    ...
    def update(cls, form_data):     
        ...
        try:
            ...
            user = cls.query.get(form_data["id"])  # Предполагаем, что есть ID пользователя
...
class Cars(db.Model):
    ...
    # # Отношения с backref
    # manager = db.relationship('Users', foreign_keys=[manager_id], backref=db.backref('managed_cars', lazy='dynamic'), lazy='select', uselist=False) 
    # buyer = db.relationship('Users', foreign_keys=[buyer_id], backref=db.backref('bought_cars', lazy='dynamic'), lazy='select', uselist=False)
    ...
    def __repr__(self):
        return f"<Cars(id={self.id}, car_number='{self.car_number}', model='{self.model}', " \
